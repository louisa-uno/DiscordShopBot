...
async def edit_item(reaction, user):
    ...
    while True:
        ...
        if edit_item_menu == "=name":
            ...
            cart_cursor.execute(f"UPDATE items SET name = '{item_name}', description = '{item_description}', url = '{item_image}', price = '{item_price}', quantity = '{item_quantity_database}' WHERE id = '{item_id}'")
            cart_database.commit()
            break
...
async def cart_ticket(database_user, reaction, user):
    ...
    if cart_exists == [(1,)]:
        ...
        embed = discord.Embed(title = config_discord["messages"]["order_title"] , description = "" , color = discord.Colour.from_rgb(255, 0, 0))
        productnames = ""
        productquantity = ""
        productprices = ""
        total = 0
...
async def cart_message(database_user, reaction, user):
    ...
    if cart_exists == [(1,)]:
        cart_cursor.execute(f"SELECT * FROM {database_user}")
        products = cart_cursor.fetchall()
        embed = discord.Embed(title = config_discord["messages"]["cart_title"] , description = "" , color = discord.Colour.from_rgb(255, 0, 0))
        productnames = ""
        productquantity = ""
        productprices = ""
        total = 0
